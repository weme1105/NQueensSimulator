...
export function installUiLayout(app: UiBridge): void {
  ...
  const settingsBtn=document.querySelector<HTMLButtonElement>('.display-settings-toggle, .settings-toggle, [data-display-settings]');
  if(settingsBtn){settingsBtn.classList.add('nq-settings-top');toolbar.insertBefore(settingsBtn,toolbar.firstChild);}
