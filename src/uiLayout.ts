...
export function installUiLayout(app: UiBridge): void {
  ...
  .nq-app-bar{display:flex;align-items:center;justify-content:flex-end;gap:8px;margin:0 0 10px;padding:8px 10px;border:1px solid #eaded8;border-radius:12px;background:#fff;min-height:48px;position:relative;flex-wrap:wrap}
  .nq-app-bar .nq-settings-button{margin-left:0!important}
  ...
  .nq-rule-tip,.nq-operation-tip{display:none}.nq-play-mode .nq-rule-tip,.nq-play-mode .nq-operation-tip{display:inline-flex}
  ...
  .nq-operation-panel{display:none;position:absolute;right:10px;top:calc(100% + 8px);z-index:120;width:min(390px,calc(100vw - 40px));padding:12px 14px;border:1px solid #eaded8;border-radius:12px;background:#fff;box-shadow:0 10px 28px #0002;line-height:1.65;font-size:13px}.nq-operation-panel.open{display:block}.nq-operation-panel b{display:block;margin-bottom:5px;font-size:14px}
  ...
  @media(max-width:850px){.nq-app-bar{padding:7px 8px;min-height:44px;gap:6px}.nq-edit-actions,.nq-play-actions{gap:6px}.nq-operation-panel{right:8px}.nq-size-picker{grid-template-columns:30px 46px 30px;grid-template-rows:21px 34px 21px;padding:2px 4px}.nq-size-current{font-size:20px}}
  `;
  document.head.append(style); document.body.classList.add('nq-source-ui'); if(mode)mode.hidden=true;

  const appBar=document.createElement('div');appBar.className='nq-app-bar';appBar.setAttribute('aria-label','應用程式功能');
  toolbar.parentElement?.insertBefore(appBar,toolbar);
  const placeGlobalControls=()=>{
    const settingsButton=document.querySelector<HTMLButtonElement>('.nq-settings-button');
    if(settingsButton&&settingsButton.parentElement!==appBar)appBar.insertBefore(settingsButton,appBar.firstChild);
  };
  placeGlobalControls();
  new MutationObserver(placeGlobalControls).observe(document.body,{childList:true,subtree:true});
  ...
  const tips=document.createElement('button');tips.type='button';tips.className='nq-rule-tip';tips.textContent='💡 規則 Tips';appBar.appendChild(tips);
  const opTip=document.createElement('button');opTip.type='button';opTip.className='nq-operation-tip';opTip.textContent='💡 操作 Tips';appBar.appendChild(opTip);
  const opPanel=document.createElement('div');opPanel.className='nq-operation-panel';opPanel.innerHTML='<b>操作說明</b><div>單點空白格：標記 X。</div><div>同一格 1 秒內連點兩下：放置皇后。</div><div>超過 1 秒：X 保留。</div><div>改點其他格：原本 X 保留，新格重新開始判定。</div><div>單點皇后：清空該格。</div><div>拖曳：連續標記或清除 X。</div>';appBar.appendChild(opPanel);opTip.addEventListener('click',()=>opPanel.classList.toggle('open'));

  const playActions=document.createElement('div');playActions.className='nq-play-actions';mainPanel.insertBefore(playActions,annotationWrap??board);playActions.append(undoBtn,clearBtn,stepBtn,autoBtn);if(guide){playActions.insertAdjacentElement('afterend',guide);guide.classList.remove('nq-open');tips.addEventListener('click',()=>guide.classList.toggle('nq-open'));}
