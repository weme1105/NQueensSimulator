...
export function installUiLayout(app: UiBridge): void {
  ...
  .nq-app-bar{display:flex;align-items:center;justify-content:flex-end;gap:8px;margin:0 0 10px;padding:8px 10px;border:1px solid #eaded8;border-radius:12px;background:#fff;min-height:48px}
  ...
  @media(max-width:850px){.nq-app-bar{padding:7px 8px;min-height:44px}.nq-edit-actions,.nq-play-actions{gap:6px}.nq-operation-panel{right:8px}.nq-size-picker{grid-template-columns:30px 46px 30px;grid-template-rows:21px 34px 21px;padding:2px 4px}.nq-size-current{font-size:20px}}
  `;
  ...
  const placeGlobalControls=()=>{
    ...
    if(settingsButton&&settingsButton.parentElement!==appBar)appBar.appendChild(settingsButton);
  };
  placeGlobalControls();
