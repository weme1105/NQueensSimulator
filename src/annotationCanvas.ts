...
export function installAnnotationCanvas(app: AnnotationBridge): void {
  ...
  const tools=document.createElement('div');tools.className='annotation-tools';tools.innerHTML=`<div class="annotation-row"><button type="button" class="annotation-tool" data-tool="freehand">✏️ 畫筆</button><button type="button" class="annotation-tool" data-tool="line">／ 直線</button><button type="button" class="annotation-tool" data-tool="arrow">→ 箭頭線</button><button type="button" class="annotation-tool" data-tool="rectangle">▭ 方框</button><label>顏色 <input class="annotation-color" type="color" value="#ff2d55"></label><label>透明度 <input class="annotation-opacity" type="range" min="10" max="100" value="75"><span class="annotation-opacity-value">75%</span></label><label>粗細 <input class="annotation-width" type="range" min="2" max="30" value="6"><span class="annotation-width-value">6</span></label></div><div class="annotation-row"><button type="button" class="annotation-eraser">橡皮擦</button><button type="button" class="annotation-undo">還原</button><button type="button" class="annotation-clear">清除</button></div>`;board.parentElement.insertBefore(tools,board);
  ...
  if(actionRow&&eraserButton){let anchor:Element=eraserButton;for(const button of [stepSolve,autoQueen]){if(!button)continue;anchor.insertAdjacentElement('afterend',button);anchor=button;}}
  ...
  const toolButtons=Array.from(tools.querySelectorAll<HTMLButtonElement>('.annotation-tool'));
  ...
  const sync=()=>{if(!app.isPlayMode())enabled=false;canvas.classList.toggle('enabled',enabled&&app.isPlayMode());for(const b of toolButtons)b.classList.toggle('active',enabled&&app.isPlayMode()&&!erase&&b.dataset.tool===tool);eraser.classList.toggle('active',enabled&&app.isPlayMode()&&erase)};
  for(const b of toolButtons)b.onclick=()=>{tool=(b.dataset.tool as ToolKind)||'freehand';enabled=true;erase=false;sync()};
  eraser.onclick=()=>{enabled=true;erase=!erase;sync()};opacity.oninput=()=>opacityValue.textContent=`${opacity.value}%`;width.oninput=()=>widthValue.textContent=width.value;undo.onclick=()=>{shapes.pop();redraw()};clear.onclick=()=>{shapes.length=0;active=null;redraw()};
