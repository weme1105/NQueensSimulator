...
export function installAnnotationCanvas(app: AnnotationBridge): void {
  ...
  for(const b of toolButtons)b.onclick=()=>{const next=(b.dataset.tool as ToolKind)||'freehand';if(enabled&&!erase&&tool===next){enabled=false;drawing=false;active=null;redraw();sync();return}tool=next;enabled=true;erase=false;sync()};
